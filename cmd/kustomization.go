...
with kubectl.`,
	Run: func(cmd *cobra.Command, args []string) {
		...
		if confirmMigrate {
			...
			// Create the ApplicationSet with the ApplicationSet Secret
			log.Info("Migrating Kustomization \"" + kustomization.Name + "\" to ArgoCD via an ApplicationSet")
			if err := utils.MigrateKustomizationToApplicationSet(k, ctx, argoCDNamespace, *kustomization, exd); err != nil {
				log.Fatal(err)
			}
